...
此外，為提升記憶體管理效率，本作業引入 **available-space list**，使節點在刪除後可被回收並重複使用，避免頻繁動態配置所造成的效能負擔。
...
2. 測試方法
 - 以多項式項數作為測試變因，分別建立不同規模之多項式物件，項數設定為 t = 10、50、100、200、500。 
 - 對每一組多項式，分別量測多項式加法、減法、乘法與代入求值之執行時間，並重複執行多次取平均值，僅統計多項式核心運算所需時間。
...
4. 效能分析
- 多項式加法與減法因採用指數遞減排序的循環鏈結串列，可透過線性歸併方式完成，其執行時間呈線性成長，符合理論時間複雜度 **O(m + n)**。
- 多項式乘法需進行雙層逐項相乘並合併同次項， 執行時間呈平方成長趨勢，為主要效能瓶頸，符合 **O(m × n)**。
- 代入求值僅需線性走訪多項式串列計算各項貢獻，效能表現穩定。
- 此外，available-space list 使節點能被回收並重複使用，有助於降低記憶體管理成本。
